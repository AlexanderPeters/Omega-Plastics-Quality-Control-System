...
public class ImagePanel extends JPanel implements Panel {
	...
	public void updateImage(BufferedImage image, Dimension programFrameSize) {
		Dimension imageScaling = new Dimension((int) (programFrameSize.width * 0.8889), (int) (programFrameSize.height * 0.6667));
		this.remove(label);
		icon = new ImageIcon(image.getScaledInstance(imageScaling.width, imageScaling.height, java.awt.Image.SCALE_SMOOTH));
		label = new JLabel("", icon, JLabel.CENTER);
		this.add(label);//, BorderLayout.CENTER);
	}
